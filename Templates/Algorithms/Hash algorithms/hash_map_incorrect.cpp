...
struct HASH_MAP{
    std::bitset<MAX_HASH_TIMES> hash_map[MAX_HASH_MOD];
    int hash_times,mod[MAX_HASH_TIMES];
    HASH_MAP(int in){
        hash_times=in;
        for(register int i=0;i<hash_times;++i) mod[i]=((i&2)?mod[i-1]-1:HASH_RAND%(MAX_HASH_MOD-MIN_HASH_MOD)+MIN_HASH_MOD);
        return;
    }
